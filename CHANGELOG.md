...
- `search --full-text` now shows a matching-context snippet for each content
  ...
  as a `snippet` field in `--json`, a fourth column in `--tsv` (tabs inside the
  snippet are flattened to spaces), and an indented line beneath the match in the
  default table. title and tag hits show an empty snippet, matching the tui
